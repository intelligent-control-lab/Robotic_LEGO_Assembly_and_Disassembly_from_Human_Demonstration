/*
***********************************************************************************************************************************************************************
This file implements the controller node.
Copyright (C) 2023
...
lego_assembly::math::VectorJd controller_goal = Eigen::MatrixXd::Zero(6, 1);
...
int main(int argc, char **argv)
{
    try
    {
        ...
        lego_assembly::udp::UDP_Interface::Ptr robot_connection = std::make_shared<lego_assembly::udp::UDP_Interface>();
        lego_assembly::math::VectorJd q;
        ...
        lego_assembly::math::VectorJd jerk_ref = Eigen::MatrixXd::Zero(6, 1);
        lego_assembly::math::VectorJd jerk_safe = Eigen::MatrixXd::Zero(6, 1);
        lego_assembly::udp::recv_pack recv_packet;
        ...
        while(ros::ok)
        {
            ...
            if(!use_robot)
            {
                usleep(7 * microsecond); // Pause <8ms to simulate the robot controller in simulation.
            }
            ros::spinOnce();
        }
        if(use_robot)
