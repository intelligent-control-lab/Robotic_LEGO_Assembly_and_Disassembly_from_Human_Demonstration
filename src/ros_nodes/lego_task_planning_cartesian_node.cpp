...
lego_assembly::math::VectorJd robot_q = Eigen::MatrixXd::Zero(6, 1);
lego_assembly::math::VectorJd robot_qd = Eigen::MatrixXd::Zero(6, 1);
lego_assembly::math::VectorJd robot_qdd = Eigen::MatrixXd::Zero(6, 1);
...
int main(int argc, char **argv)
{
    try
    {
        ...
        lego_assembly::lego::Lego_Gazebo::Ptr lego_gazebo_ptr = std::make_shared<lego_assembly::lego::Lego_Gazebo>();
        lego_gazebo_ptr->setup(gazebo_env_setup_fname, assemble, task_json);
      
        int robot_dof = 6;
        lego_assembly::robot::Robot::Ptr robot = std::make_shared<lego_assembly::robot::Robot>();
        ...
        int pre_mode = -1;
        lego_assembly::math::VectorJd cur_goal = home_q;
        ...
        while(ros::ok)
        {
            ...
            if(robot->reached_goal(cur_goal) && robot->is_static())
            {
                ...
                else if(mode == 1)
                {
                    ...
                    cur_goal =  lego_assembly::math::IK_closed_form(cur_goal, up_T, robot->robot_DH_tool(), 
                                                                          robot->robot_base_inv(), robot->robot_tool_inv(), 0, IK_status);
                }
                else if(mode == 2)
                {
                    // cart_T(2, 3) = cart_T(2, 3) - 0.002;
                    cur_goal =  lego_assembly::math::IK_closed_form(cur_goal, cart_T, robot->robot_DH_tool(), 
                                                                          robot->robot_base_inv(), robot->robot_tool_inv(), 0, IK_status);
                }
                else if(mode == 3)
                {
                    ...
                    cart_T = lego_assembly::math::FK(cur_goal, robot->robot_DH_tool_disassemble(), robot->robot_base(), false);
                    cart_T = cart_T * twist_T;
                    cur_goal =  lego_assembly::math::IK_closed_form(cur_goal, cart_T, robot->robot_DH_tool_disassemble(), 
                                                                          robot->robot_base_inv(), robot->robot_tool_disassemble_inv(),0,IK_status);
                }
                else if(mode == 4 || mode == 9)
                {
                    cart_T = lego_assembly::math::FK(cur_goal, robot->robot_DH_tool_assemble(), robot->robot_base(), false);
                    cart_T(2, 3) = cart_T(2, 3) + 0.015;
                    cur_goal =  lego_assembly::math::IK_closed_form(cur_goal, cart_T, robot->robot_DH_tool_assemble(), 
